...
wait(int *wstatus)
{
	return __syscall1(SYS_wait, (long)wstatus);
}

int
pipe(int pipefd[2])
{
	return __syscall1(SYS_pipe, (long)pipefd);
}
...
{
	return __syscall3(SYS_execve, (long)pathname, (long)argv, (long)envp);
}
...
{
	return __syscall1(SYS_unlink, (long)pathname);
}

int
link(const char *oldpath, const char *newpath)
{
	return __syscall2(SYS_link, (long)oldpath, (long)newpath);
}
...
dup(int oldfd)
{
	return __syscall1(SYS_dup, oldfd);
}
...
{
	return __syscall1(SYS_alarm, seconds);
}

time_t
time(time_t *tloc)
{
	return __syscall1(SYS_time, (long)tloc);
}
...
{
	return __syscall2(SYS_symlink, (long)target, (long)linkpath);
}

ssize_t
readlink(const char *restrict pathname, char *restrict linkpath, size_t buf)
{
	return __syscall3(SYS_readlink, (long)pathname, (long)linkpath, buf);
}
...
getcwd(char *buf, size_t n)
{
	...
	int ret = __syscall2(SYS_getcwd, (long)buf, n);
	if (ret < 0)
		return NULL;
	else
		return buf;
...
vfork(void)
{
	return __syscall0(SYS_vfork);
}

clock_t
times(struct tms *buf)
{
	return __syscall1(SYS_times, (long)buf);
}

int
access(const char *pathname, int mode)
{
	return __syscall2(SYS_access, (long)pathname, mode);
}
