...
{
	return (sighandler_t)__syscall2(SYS_signal, signum, (long)handler);
}
...
{
	return __syscall3(SYS_sigprocmask, how, (long)set, (long)oldset);
}

int
sigsuspend(const sigset_t *mask)
{
	return __syscall1(SYS_sigsuspend, (long)mask);
}

int
sigaction(int signum, const struct sigaction *restrict act, struct sigaction *restrict oldact)
{
	return __syscall3(SYS_sigaction, signum, (long)act, (long)oldact);
}
