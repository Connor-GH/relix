...
truncate1(char *s)
{
	...
	int n = read(fd2, buf, sizeof(buf1));
	if (n != 4) {
...
writebig(char *s)
{
	...
	for (size_t i = 0; i < __MAXFILE; i++) {
		((int *)buf)[0] = i;
		if (write(fd, buf, __BSIZE) != __BSIZE) {
			printf("%s: error: write big file failed i=%zu\n", s, i);
			exit(1);
		}
	}
...
bigwrite(char *s)
{
	...
	unlink("bigwrite");
	for (size_t sz = 499; sz < (MAXOPBLOCKS + 2) * __BSIZE; sz += 471) {
...
sbrkfail(char *s)
{
	...
	if (pid == 0) {
		...
		sbrk(10 * BIG);
		int n = 0;
		for (size_t i = 0; i < 10 * BIG; i += PGSIZE) {
			n += *(a + i);
		}
...
diskfull(char *s)
{
	...
	for (fi = 0; done == 0 && '0' + fi < 0177; fi++) {
		...
		for (size_t i = 0; i < __MAXFILE; i++) {
			char buf1[__BSIZE];
			if (write(fd, buf1, __BSIZE) != __BSIZE) {
				done = 1;
				close(fd);
				break;
			}
